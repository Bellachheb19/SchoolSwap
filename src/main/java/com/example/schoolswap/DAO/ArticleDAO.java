...
public class ArticleDAO {
    ...
    public List<Article> getAnnoncesRecentes() {
        ...
        try (Session session = HibernateUtil.getSessionFactory().openSession()) {
            ...
            Query<Article> query = session.createQuery("SELECT a FROM Article a LEFT JOIN FETCH a.idCat LEFT JOIN FETCH a.idUser", Article.class);
...
public static void createarticle(Article article) throws Exception {
    ...
    try {
        ...
        // Attacher les entités issues d'une autre session (Hibernate)
        if (article.getIdCat() != null && article.getIdCat().getId() != null) {
            article.setIdCat(session.getReference(Categorie.class, article.getIdCat().getId()));
        }
        if (article.getIdUser() != null && article.getIdUser().getId() != null) {
            article.setIdUser(session.getReference(com.entity.User.class, article.getIdUser().getId()));
        }
        ...
        throw e; // Propage l'erreur pour l'afficher sur l'écran
    } finally {
        session.close();
    }
}
    ...
    public static void deletearticle(Integer id) throws Exception {
        Session session = HibernateUtil.getSessionFactory().openSession();
        Transaction tx = null;
        try {
            tx = session.beginTransaction();
